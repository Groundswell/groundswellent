...
module PrintfulClientModule
    ...
    def load_products
        self.get( 'products' ).each do |origin_prod|
            prod = Product.where( 
                origin_product_id: origin_prod['id'], origin: 'printful' ).first_or_initialize( 
                    title: origin_prod['model'],
                    brand: origin_prod['brand'], 
                    product_type: origin_prod['type'],
                    avatar: origin_prod['image'] 
            )
            prod.save
            puts "Saved prod: #{prod.title}"
        end

        Product.where( origin: 'printful' ).each do |prod|
            self.get( "products/#{prod.origin_product_id}" )['variants'].each do |var|
                pv = prod.product_variants.where( 
                    origin_variant_id: var['id'] ).first_or_initialize( 
                        title: var['name'], 
                        avatar: var['image'], 
                        origin_price: var['price'].gsub( '.', '' ), 
                        size: var['size'], 
                        color: var['color'], 
                        color_code: var['color_code'] )
                if pv.title.match( /Men/ )
                    pv.gender = 'male'
                elsif pv.title.match( /Women/ )
                    pv.gender = 'female'
                end
                pv.save
                puts "Saved Variant #{pv.title}"
            end
        end
    end
